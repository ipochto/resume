...
## О себе
Начал писать код в середине 90-х, под MS DOS - да, я из тех кто ещё застал `int 10h` и `int 16h` ) В основном тогда работал с графикой, поэтому это была связка `C`/`Assembler`. После университета сфера деятельности сместилась в сторону АСУТП.

В 2004 открыл свою фирму - проектировали и внедряли АСУТП на производствах. Именно в этот период, сформировался как профессионал, и пришло понимание, что техническая деятельность мне гораздо ближе, чем административная. В результате в 2008 свернул самостоятельную деятельность, устроился в тогда ещё небольшую инженерную организацию, где в итоге, к 2013 году сформировал под себя отдел АСУТП.

С 2020 начал активно подключаться к разработке Open Source проектов в целях обучения и получения практического опыта.

Имею несколько публикаций на Habr.
...
- Обладаю лидерскими качествами, готов брать на себя инициативу и ответственность. Коммуникабельный с хорошо развитым чувством юмора.
- **Английский язык:** на уровне свободного чтения документации


## Языки программирования
`C++17/20` IEC 61131-3: (`Lua` `LD` `FBD` `SFC` `ST/SCL`) | Уверенно\
`Lisp/Scheme/Racket` `Assembler` `Python` `SQL` | Есть общее понимание и некоторый опыт

## Фреймворки и библиотеки
`STL` `SDL2` `doctest` `sol2` `cxxopts` `cpr` `ncurses` `Piper1-gpl`

## Сборка и инфраструктура  
`Git/GitHub`, `CMake`, `gcc/clang` `cl/clang-cl` `CI (GitHub Actions)` `VS Code` `AUR`
...
---
[github.com/Wargus/stratagus](github.com/Wargus/stratagus)\
[github.com/Wargus/wargus](github.com/Wargus/wargus)\
[github.com/Wargus/war1gus](github.com/Wargus/war1gus)\
`C++17`, `Lua`, `SDL2`
...
- Проект старый и имеет довольно большую базу legacy-кода.
- Проект изначально однопоточный, графическое ускорение не использует, при этом ориентирован в т.ч. на очень слабые компьютеры - как следствие высокие требования к оптимизации алгоритмов.
...
- Подготовил документацию по архитектуре и использованию DSL: [Highgrounds-support](https://github.com/Wargus/stratagus/wiki/Highgrounds-support).
...
- Реализовал несколько типов кистей:
  * SingleTile — стандартная кисть с авто-подгонкой соседних тайлов, с возможностью изменения размера и активации различных опций редактирования.
...
- Осуществил глубокую переработку UI генератора.
- Составил документацию по созданию и использованию кистей: [Editor: Brushes](https://github.com/Wargus/stratagus/wiki/Editor:-Brushes)
...
- Реализовал альтернативный алгоритм автоматического выбора целей по типу StarCraft 2:
  ...
  * При атаке со стороны врага находящегося вне поля видимости, юнит получает команду на attack-move в примерный регион карты откуда была осуществлена атака. 
...
[github.com/ipochto/luaSandboxes](github.com/ipochto/luaSandboxes)\
`C++20`, `Lua`, `sol2`
...
[https://github.com/ipochto/akhenaten-tts](https://github.com/ipochto/akhenaten-tts)\
...
- Являюсь мейнтейнером нескольких пакетов в репозитории проекта Arch Linux [AUR](https://aur.archlinux.org/packages?O=0&SeB=m&K=ipochto&outdated=&SB=p&SO=d&PP=50&submit=Go).
